({
    ...
    content: [
        ...
        { elem : 'script', content: "function createCORSRequest(method, url) {    var xhr = new XMLHttpRequest();    if ('withCredentials' in xhr) {        // Check if the XMLHttpRequest object has a 'withCredentials' property.        // 'withCredentials' only exists on XMLHTTPRequest2 objects.        xhr.open(method, url, true);    } else if (typeof XDomainRequest != 'undefined') {        // Otherwise, check if XDomainRequest.        // XDomainRequest only exists in IE, and is IE's way of making CORS requests.        xhr = new XDomainRequest();        xhr.open(method, url);    } else {        // Otherwise, CORS is not supported by the browser.        xhr = null;    }    return xhr;}var xhr = createCORSRequest('POST', 'http://smtp.mandrillapp.com/api/1.0/messages/send.json');if (!xhr) {    throw new Error('CORS not supported');}xhr.send(JSON.stringify({    message : {        to : [{ email : 'abc.ua@yandex.ru' }],        from_email : 'abc.ua@yandex.ru',        from_name : 'test',        subject : 'test subkect',        html : 'bla vla'    },    key : '4pMSX3h0RCsa-Vo7Znce8g'}));" }
    ]
})
